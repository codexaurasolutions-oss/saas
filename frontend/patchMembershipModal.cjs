const fs = require('fs');
let code = fs.readFileSync('src/pages/owner/PosPage.jsx', 'utf8');
...
// 3. Update button
code = code.replace('onClick={() => setTab("memberships")}>Add Membership</button>', 'onClick={() => setShowMembershipModal(true)}>Add Membership</button>');

// 4. Update custom membership rendering logic
code = code.replace(
  'membershipLookup[item.membershipPlanId]',
  '(item.membershipPlanId === "CUSTOM" ? { name: item.name, price: item.unitPrice } : membershipLookup[item.membershipPlanId])'
);
          ...
          <div style={{ background: "white", borderRadius: "12px", width: "900px", maxWidth: "95vw", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: "20px", overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "12px", overflowX: "auto", paddingBottom: "8px" }} className="no-scrollbar">
                {(context.memberships || []).filter(p => !membershipSearch || p.name.toLowerCase().includes(membershipSearch.toLowerCase())).map(pkg => {
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1fr", gap: "12px" }}>
                ...
                <div>
                  ...
                  <select value={membershipDraft.staffId} onChange={(e) => setMembershipDraft({...membershipDraft, staffId: e.target.value})} style={{ width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #cbd5e1", fontSize: "0.75rem", outline: "none", height: "36px", boxSizing: "border-box", background: "white" }}>
                    ...
                    {context.staffUsers.map(s => <option key={s.id} value={s.id}>{s.user?.name}</option>)}
                  </select>
                </div>
                <div>
...
fs.writeFileSync('src/pages/owner/PosPage.jsx', code);
